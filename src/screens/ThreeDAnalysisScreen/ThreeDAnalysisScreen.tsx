...
function deduplicateFeatures(features: any[], tapLng: number, tapLat: number): any | null {
  ...
  for (const f of features) {
    ...
    const containsTap = pointInGeometry(tapLng, tapLat, f.geometry);
    const centroid = buildingCentroid(f.geometry);
    const distToCentroid = centroid
      ? Math.sqrt(Math.pow((centroid[0] - tapLng) * METERS_PER_DEG, 2) + Math.pow((centroid[1] - tapLat) * METERS_PER_DEG, 2))
      : Infinity;

    candidates.push({ feature: f, area, containsTap, distToCentroid });
  }
  ...
  if (containing.length > 0) {
    ...
    return containing[0].feature;
  }

  candidates.sort((a, b) => a.distToCentroid - b.distToCentroid);
...
export function ThreeDAnalysisScreen(): JSX.Element {
  ...
  useEffect(() => {
    ...
    ctx.fill();
  }, [sunPosition, sunVisible, mapReady, sunTimes, lat, lng, hasCoords, viewVersion, center[0], center[1], focusedBuilding]);
  ...
  useEffect(() => {
    ...
    if (focusedBuilding && focusedBuilding.geometry) {
      // Highlight layer (semi-transparent shell)
      try {
    ...
    // Reset main building layer to grey
    try {
  ...
  return (
    <main className="relative h-[100dvh] w-full overflow-hidden bg-white">
      <div ref={containerRef} className={`absolute inset-0 h-full w-full ${focusMode ? "cursor-crosshair" : ""}`} />
        ...
        {mapReady && (
          ...
          >
            ...
            <span className="font-['Inter'] text-sm font-medium">
              {focusMode ? t("threed.focusBuildingActive") : t("threed.focusBuilding")}
            </span>
          </Button>
        )}
      </header>
