...
// Helper to format the short date if not imported from utils
...
export function ThreeDAnalysisScreen(): JSX.Element {
  ...
  const sunTimes = useMemo<SunTimes & { noon?: Date }>(() => {
    if (!hasCoords) return null;
    const times = getSunTimes(selectedDate, lat, lng);
    // Attach solar noon from SunCalc if needed by your UI
    const scTimes = SunCalc.getTimes(selectedDate, lat, lng);
    return { ...times, noon: scTimes.solarNoon };
  }, [selectedDate, lat, lng, hasCoords]);
        ...
        >
          ...
          {sunTimes ? (
            <div className="mb-1 flex items-center justify-between font-['Inter'] text-[11px] text-[#828282]">
              <span>{t("analysis.sunrise")} {sunTimes.sunrise ? formatClock(sunTimes.sunrise, locationTimeZone) : "--:--"}</span>
              <span>{t("analysis.solarNoon")} {sunTimes.noon ? formatClock(sunTimes.noon, locationTimeZone) : "--:--"}</span>
              <span>{t("analysis.sunset")} {sunTimes.sunset ? formatClock(sunTimes.sunset, locationTimeZone) : "--:--"}</span>
            </div>
          ) : (
